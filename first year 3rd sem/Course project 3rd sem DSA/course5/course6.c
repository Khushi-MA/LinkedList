...
int main()
{
    ...
    printf("\n%s...%s", s, d);
    for(j=1;j<=10;j++)
        if(strcasecmp(s,cityy[j].name)==0)
            source_city = j;

    for(k=1;k<=10;k++)
        if(strcasecmp(d,cityy[k].name)==0)
             dest_city=k;
    ...
    while(choice!=100)
    {
        ...
        switch(choice)
        {
            ...
            case 6: // "testing when running" "while loop"
                    ...
                    airways(source_city,dest_city,flights,d);
                    break;

            case 5: hotel_rating(d,hot_details);
            ...
            case 8: // Function call for dihikstra
                    roaddijkstra(VERT, roadway, 1);
                    break;

            case 9: flightdijikstra(air,11,source_city,dest_city);
                    break;

            case 10: airways(source_city,dest_city,flights,d);
                    break;

            case 11: places_roam(d,roam);
                    break;
